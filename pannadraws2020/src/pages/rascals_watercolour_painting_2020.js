...
const Rascals = (props) => {
  const imgData = useStaticQuery(graphql`
    query {
        queryData: file(relativePath: { eq: "2020/rascals_2020_web.jpg" }) {
          childImageSharp {
            fluid {
   ...
   `);
  const imgMeta = animalArtMeta.rascals;

  return (
      <Layout>
        <SEO title="Rascals watercolour painting" />
